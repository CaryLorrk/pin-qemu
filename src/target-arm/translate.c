...
#ifdef CONFIG_PIN
static void arm_insn_analysis(CPUARMState * env, DisasContext *s, INS ins)
{
    unsigned int cond, insn, val, op1, i, shift, rm, rs, rn, rd, sh;
    TCGv_i32 tmp;
    TCGv_i32 tmp2;
...
static inline void bbl_analysis (ARMCPU *cpu,
                                 TranslationBlock *tb,
                                 BBL bbl)
{
    CPUState *cs = CPU(cpu);
    ...
    do {
        ...
        if (dc->thumb) {
            ...
            INS ins = ins_create();
            bbl_addIns(bbl, ins);
            arm_insn_analysis(env, dc, ins);
            callInsInstrumentFunctions(ins);
        }
...
done_generating:
    ...
    *tcg_ctx.gen_opc_ptr = INDEX_op_end;
    ...
    tb->size = dc->pc - pc_start;
    tb->icount = num_insns;
    bbl->numInsns = num_insns;
}

void gen_calls(CPUARMState *env, AFUNPTR func, 
               GArray *iargs, size_t n_args, INS ins)
{
    TCGv_ptr fn;
    TCGArg *args = g_malloc(n_args*sizeof(TCGArg));
    int *tag = g_malloc(n_args*sizeof(int));
    TCGv_i32 *temp_i32 = g_malloc(n_args*sizeof(TCGv_i32));
    TCGv_i64 *temp_i64 = g_malloc(n_args*sizeof(TCGv_i64));
    TCGv_ptr *temp_ptr = g_malloc(n_args*sizeof(TCGv_ptr));
    int n = 0, niargs = 0;
    
    while (n < n_args)
    {
        IARG_TYPE iarg = (IARG_TYPE)(g_array_index(iargs,int,niargs++));
        switch(iarg)
        {
            case IARG_ADDRINT:
                tag[n] = 0;
                temp_i32[n] = tcg_const_i32(
                                g_array_index(iargs,int,niargs++));
                args[n] = GET_TCGV_I32(temp_i32[n]);
                break;
            case IARG_PTR:
                tag[n] = 2;
                temp_ptr[n] = tcg_const_ptr(
                                g_array_index(iargs,int,niargs++));
                args[n] = GET_TCGV_PTR(temp_ptr[n]);
                break;
            case IARG_BOOL:
                tag[n] = 0;
                temp_i32[n] = tcg_const_i32(
    ...
    tcg_gen_callN(&tcg_ctx, fn, 0, 0, ((TCGArg)(-1)),
            n_args, args);
    ...
    for (n = 0; n < n_args; ++n)
    {
        ...
        {
            case 0:
                tcg_temp_free_i32(temp_i32[n]);
                break;
            case 1:
                tcg_temp_free_i64(temp_i64[n]);
                break;
            case 2:
                tcg_temp_free_ptr(temp_ptr[n]);
                break;
        }
    }
}

void gen_before_calls(CPUARMState *env, INS ins)
{
    int i;
    for (i = 0; i < ins->n_beforeCalls; ++i)
    {
        AFUNPTR funptr = g_array_index(ins->beforeCalls[0],
                                       AFUNPTR, i);
        GArray *iargs = g_array_index(ins->beforeCalls[1],
                                      GArray*, i);
        size_t n_args = g_array_index(ins->beforeCalls[2],
                                      size_t, i);


        gen_calls(env, funptr, iargs, n_args, ins);
    }
}

static inline void bbl_gen_code (ARMCPU *cpu,
                                 TranslationBlock *tb,
                                 BBL bbl)
{
    CPUState *cs = CPU(cpu);
...
{
    BBL bbl = bbl_create();
    TCGContext tcg_ctx_temp = tcg_ctx;
    bbl_analysis(arm_env_get_cpu(env), tb, bbl);
    tcg_ctx = tcg_ctx_temp;
    bbl_gen_code(arm_env_get_cpu(env), tb, bbl);
    bbl_delete(bbl);
}
#else
void gen_intermediate_code(CPUARMState *env, TranslationBlock *tb)
